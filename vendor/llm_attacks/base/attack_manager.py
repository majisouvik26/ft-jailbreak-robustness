...
# Templates that use incremental re-tokenization for slice alignment (llama-2 path).
_SLICE_TEMPLATE_NAMES = frozenset({
    'llama-2', 'llama-3', 'qwen-chatml', 'phi-chat', 'gemma-chat',
})


def _resolve_embed_module(model):
    """Resolve embedding module for Llama, Qwen, Phi, Gemma, GPT, etc."""
    ...
    raise ValueError(f"Unknown model type for embeddings: {type(model)}")


def get_embedding_layer(model):
    return _resolve_embed_module(model)


def get_embedding_matrix(model):
    return _resolve_embed_module(model).weight


def get_embeddings(model, input_ids):
    embed = _resolve_embed_module(model)
    out = embed(input_ids)
...
class AttackPrompt(object):
    ...
    def _update_ids(self):
        ...
        if self.conv_template.name in _SLICE_TEMPLATE_NAMES:
            self.conv_template.messages = []
        ...
        if isinstance(test_controls, torch.Tensor):
            ...
            if hasattr(torch, 'nested') and hasattr(torch.nested, 'nested_tensor'):
                ...
                test_ids = torch.nn.utils.rnn.pad_sequence(
                    [t if t.dim() == 1 else t.squeeze(0) for t in test_ids],
                    batch_first=True,
...
class ModelWorker(object):
    ...
    def __init__(self, model_path, model_kwargs, tokenizer, conv_template, device):
        load_kwargs = dict(trust_remote_code=True, **model_kwargs)
...
def get_workers(params, eval=False):
    ...
    for i in range(len(params.tokenizer_paths)):
        ...
        if 'guanaco' in params.tokenizer_paths[i]:
            ...
            tokenizer.unk_token_id = 0
        if any(x in params.tokenizer_paths[i].lower() for x in ('llama', 'meta-llama')):
    ...
    for conv in raw_conv_templates:
        if conv.name == 'zero_shot':
            ...
            conv.sep = '\n'
        elif conv.name in _SLICE_TEMPLATE_NAMES and getattr(conv, 'sep2', None):
